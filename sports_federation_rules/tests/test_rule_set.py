...
class TestFederationRuleSet(TransactionCase):
    ...
    def test_competition_count(self):
        """Test competition count computation."""
        self.assertEqual(self.rule_set.competition_count, 0)
        comp = self.env["federation.competition"].create({
            "name": "Test League",
            "competition_type": "league",
            "rule_set_id": self.rule_set.id,
        })
        self.rule_set._compute_competition_count()
        self.assertEqual(self.rule_set.competition_count, 1)

    def test_tournament_rule_set_link(self):
        """Test that tournaments can reference a rule set via competition."""
        comp = self.env["federation.competition"].create({
        ...
        self.assertEqual(tournament.rule_set_id, self.rule_set)
        self.assertEqual(tournament.competition_id, comp)

    def test_season_registration_rule_set_link(self):
        """Test that season registrations can reference competition and rule set."""
        comp = self.env["federation.competition"].create({
            "name": "Division 1",
            "competition_type": "league",
            "rule_set_id": self.rule_set.id,
        })
        season = self.env["federation.season"].create({"name": "2025-2026"})
        club = self.env["federation.club"].create({"name": "Test Club"})
        ...
        self.assertEqual(reg.competition_id, comp)
